...
// Quiz bíblico de múltipla escolha pra 6-12 anos — testa o que já aprendeu
// nas histórias, diferente do "Complete o Versículo" que é sobre memorização.
function QuizGame({ item, color, onCorrect }) {
  ...
  const pick = (opt) => {
    ...
    setPicked(opt);
    if (opt === item.answer) onCorrect && onCorrect();
  };

  return (
    <div>
      <p style={{ fontFamily: "Fraunces", color: "#3A2E22" }} className="text-[16px] leading-relaxed text-center mb-6">{item.question}</p>
